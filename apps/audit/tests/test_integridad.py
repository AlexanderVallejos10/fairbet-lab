...
def market_with_selection(db):
    event = Event.objects.create(name='Peru vs Chile',sport='futbol',starts_at=timezone.now() + timezone.timedelta(days=1),)
    market = Market.objects.create(event=event,name='Resultado final',market_type=Market.Type.UNO_X_DOS,)
    selection = Selection.objects.create(market=market,name='local',odds=Decimal('2.1000'),)
    return market, selection

@pytest.mark.django_db
def test_primer_registro():
    record = AuditLog.objects.create(event_type='wallet.deposit',payload={'amount': '10.0000'},)
...
def test_signal_wallet_crea_auditlog(regular_user):
    ...
    assert {record.payload['direction'] for record in records} == {Direction.CREDIT,Direction.DEBIT,}
...
def test_dashboard_endpoint_admin(client, admin_user):
    ...
    assert response.status_code == 200
    assert set(response.data) == {'ggr','total_bets','total_bets_won','total_bets_lost','total_bets_pending','active_users','exposure_by_event',}
