...
class EventAdmin(admin.ModelAdmin):
    list_display = ("sport", "home_team", "away_team", "start_at", "status", "result", "max_total_bets", "max_event_exposure")
...
class OddAdmin(admin.ModelAdmin):
    list_display = ("event", "selection", "decimal_odds", "max_selection_exposure", "is_active", "updated_at")
    list_filter = ("is_active", "selection")
    search_fields = ("event__home_team", "event__away_team")

@admin.register(Bet)
class BetAdmin(admin.ModelAdmin):
    list_display = ("ticket_code", "user", "event", "selection", "stake", "status", "payout", "placed_at")
