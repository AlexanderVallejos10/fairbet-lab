...
class EventSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Event
        fields = ( "id", "home_team", "away_team", "start_at", "status", "result", "odds", "created_at",)
class BetSerializer(serializers.ModelSerializer):
    event = EventSerializer(read_only=True)
    odd_id = serializers.IntegerField(source="odd.id", read_only=True)
    class Meta:
        model = Bet
        fields = ("id", "transaction_id", "idempotency_key", "event", "odd_id", "selection","stake", "odds_snapshot", "status", "payout", "placed_at", "settled_at",)
